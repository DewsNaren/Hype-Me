...
const saveProductsBtn = document.querySelector(
  ".add-product-outer-container .save-btn-container .save-btn"
);
const submitProductsBtn = document.querySelector(
  ".add-product-outer-container .save-btn-container .submit-btn"
);
const updateLocalProductBtn = document.querySelector(
  ".add-product-outer-container .save-btn-container .save-local-btn"
);
...
const uploadImgBtn = document.querySelector(
  ".image-upload-container .upload-img-btn"
);
const imgEditBtn = document.querySelector(
  ".add-product-inner-container .image-upload-container .image-header-container button"
);
const productModalSaveBtn = document.querySelector(
  ".product-modal-container .product-modal-content .modal-footer .save-btn"
);
if (uploadImgBtn) {
...
const productModalMainImageContainer = productModalContainer.querySelector(
  ".product-img-wrapper .product-img-container .main-img-container"
);
const prodMainImgWrapper =
  productModalMainImageContainer.querySelector(".main-img-wrapper");
let productThumbEditBtn = productModalMainImageContainer.querySelector(
  ".edit-thumb-img-btn"
);
let productThumbDeleteBtn = productModalMainImageContainer.querySelector(
  ".delete-thumb-img-btn"
);
const productModalFooterContent =
  productModalContainer.querySelector(".modal-footer p");
const productModalCloseBtn = document.querySelector(
  ".product-modal-container .product-modal-content .product-modal-header .modal-close-btn"
);
const uploadImgInputs = productModalContainer.querySelectorAll(
  ".product-img-wrapper .product-img-container .thumbnail-img-container .upload-img-input"
);

let selectedFiles = [];
let initialThumbSrcs = [];
let initialMainImgSrc;
productModalSaveBtn.setAttribute("disabled", "true");
productModalSaveBtn.classList.remove("active");
...
uploadImgInputs.forEach((input, index) => {
  input.addEventListener("change", (event) => {
    ...
    thumbnail.insertAdjacentHTML(
      "beforeend",
      `
      <img src="./assets/images/${imageUrl}" alt="uploaded image" class="thumb-${
        index + 1
      }">
...
const thumbnailContainers = document.querySelectorAll(
  ".product-img-wrapper .product-img-container .thumbnail-img-wrapper .thumbnail-img-container .thumbnail-img"
);
...
thumbnailContainers.forEach((thumbnail) => {
  ...
  thumbnail.addEventListener("dragend", () => {
    ...
    changeHeaderText();
    const thumb1 = document.querySelector(".thumbnail-img-wrapper .thumb-1");
    ...
    if (thumb1 && mainImg) {
      ...
      ProductImgSaveBtn.classList.remove("active");
      ProductImgSaveBtn.setAttribute("disabled", true);
      productModalFooterContent.childNodes[1].textContent =
        "Mandatory product images (0/1) uploaded";
    }
  });
  ...
  thumbnail.addEventListener("dragover", (e) => {
    ...
    if (offset > 0) {
      wrapper.insertBefore(
        ...
        draggingOver.parentNode.nextSibling
      );
    } else {
...
const labels = [
  "Front View <span class='star'>*</span>",
  "Side View",
  "Side / Top",
  "Top / Bottom",
];
...
function updateThumbnailLabels() {
  ...
  containers.forEach((container, index) => {
    ...
    if (p) {
      p.innerHTML = labels[index] || `View ${index + 1}`;
    }
  });
}
...
function changeHeaderText() {
  thumbnailContainers.forEach((thubImg) => {
    ...
    const imgTextHeader = document.querySelector(
      ".product-img-wrapper .product-img-desc-container h3"
    );
...
//   const thumbImgs = document.querySelector(".thumbnail-img-container .thumbnail-img");
//   let clickedThumb;
//   thumbImgs.forEach(thumbImg=>{
//     if(thumbImg.classList.contains("clicked"))
//       clickedThumb=thumbImg
//   })
//   if (clickedThumb) {
//     const mainImg = prodMainImgWrapper.querySelector(".main-img");
//     if (mainImg) {
...
function updateSelectedFilesOrder() {
  ...
  containers.forEach((container, newIndex) => {
    const thumbnailImgDiv = container.querySelector(".thumbnail-img");
    const thumbnailImgTag = container.querySelector("img[class^='thumb-']");
    const originalIndex = thumbnailImgDiv?.dataset.index;

    if (originalIndex !== undefined && selectedFiles[originalIndex]) {
      reorderedFiles[newIndex] = selectedFiles[originalIndex];
      thumbnailImgDiv.dataset.index = newIndex;
    }

    if (thumbnailImgTag) {
      thumbnailImgTag.className = thumbnailImgTag.className.replace(
        /thumb-\d+/g,
        ""
      );

      thumbnailImgTag.classList.add(`thumb-${newIndex + 1}`);
    }
  });

  // overwrite selectedFiles with reorderedFiles
  selectedFiles.length = 0;
  reorderedFiles.forEach((file, i) => {
    if (file) selectedFiles[i] = file;
  });
...
function initThumbnailClickHandler() {
  const container = document.querySelector(
    ".product-img-wrapper .product-img-container .thumbnail-img-wrapper"
  );
  if (!container) return;
...
const thumbnailPreview = document.querySelector(
  ".image-upload-container .thumbnail-preview-container"
);
...
const mainImageContainer = document.querySelector(
  ".image-upload-container .main-image-container"
);

const productImgEditBtn = document.querySelector(
  ".add-product-inner-container .image-upload-container .image-header-container .edit-product-img-btn"
);
const ProductImgStatImg = document.querySelector(
  ".add-product-inner-container .image-upload-container .image-header-container img"
);
const ProductImgStat = document.querySelector(
  ".add-product-inner-container .image-upload-container .image-header-container p span"
);
...
function saveProductImages() {
  ...
  const labels = productModalContainer
    .querySelector(".thumbnail-img-wrapper")
    .querySelectorAll("label");
  labels.forEach((label) => {
    const next = label.nextElementSibling;

    const isThumbImg = next?.matches('img[class^="thumb-"]');
...
function removeProductModalDatas() {
  const productMainImg = document.querySelector(".main-img-wrapper .main-img");

  const thumbnailImgContainer = productModalContainer.querySelectorAll(
    ".thumbnail-img-wrapper .thumbnail-img-container .thumbnail-img"
  );
...
const productImgCancelBtn = document.querySelector(
  ".product-modal-container .product-modal-content .modal-footer .cancel-btn"
);
...
function storeInitialImages() {
  ...
  containers.forEach((container, index) => {
    ...
    if (img) initialThumbSrcs[index] = img.src;
  });
...
//   containers.forEach((container, index) => {
//     const img = container.querySelector("img[class^='thumb-']");
//     if (img && initialThumbSrcs[index]) {
//       img.src = initialThumbSrcs[index];
//     }
//   });

//   const mainImg = prodMainImgWrapper.querySelector(".main-img");
//   if (mainImg && initialMainImgSrc) {
//     mainImg.src = initialMainImgSrc;
//   }
// }
...
function restoreInitialImages() {
  ...
  containers.forEach((container, index) => {
    const img = container.querySelector("img[class^='thumb-']");
    if (initialThumbSrcs[index]) {
      if (img) {
        img.src = initialThumbSrcs[index];
      } else {
        const html = `
        <img src="${
          initialThumbSrcs[index]
        }" alt="uploaded image" class="thumb-${index + 1}">
        <span><img src="./assets/images/published.png" alt="published"></span>
      `;
        const thumbImg = container.querySelector(".thumbnail-img");
        thumbImg.insertAdjacentHTML("beforeend", html);
        const label = thumbImg.querySelector("label");
        label.classList.add("not-active");
      }
    } else {
      const notFoundimg = container.querySelector(
        `img[class^='thumb-${index + 1}']`
      );
      if (notFoundimg) {
        const parent = notFoundimg.parentElement;
  ...
  if (mainImg && initialMainImgSrc) {
    ...
    prodMainImgWrapper.innerHTML = `<img src="${initialMainImgSrc}" class="main-img" alt="main image">`;
  }
}
...
if (imgEditBtn) {
  imgEditBtn.addEventListener("click", () => {
    ...
    //   const previewImages=document.querySelectorAll(".thumbnail-preview-container .thumbnail-preview-img-container .thumbnail-preview-img")
    //   let previewImgSrcs=[];
    //   previewImages.forEach(previewImg=>{
    //     previewImgSrcs.push(previewImg.src)
    ...
    if (isLocalProduct) {
      const mainImg = document.querySelector(".main-image");
      prodMainImgWrapper.innerHTML = `
    <img src="${mainImg.src}" class="main-img" alt="main image">
  `;

      if (!window.hasEditedOnce) {
        const previewImages = document.querySelectorAll(
          ".thumbnail-preview-container .thumbnail-preview-img-container .thumbnail-preview-img"
        );

        const previewImgSrcs = Array.from(previewImages).map((img) => img.src);

        const thumbnailImgsWrapper = productModalContainer.querySelector(
          ".thumbnail-img-wrapper"
        );
        const thumbnailImgContainer =
          thumbnailImgsWrapper.querySelectorAll(".thumbnail-img");

        thumbnailImgContainer.forEach((thumbnailImg, index) => {
          thumbnailImg.classList.remove("clicked");

          const existingImg = thumbnailImg.querySelector(
            "img[class^='thumb-']"
          );
          const existingSpan = thumbnailImg.querySelector("span");
          const thumbLabel = thumbnailImg.querySelector("label");
...
function enableProductModalSave() {
  ...
  const mainImg = productModalContainer.querySelector(
    ".main-img-container .main-img-wrapper .main-img"
  );
  const productModalSaveBtn = document.querySelector(
    ".product-modal-container .product-modal-content .modal-footer .save-btn"
  );
...
function editThumbImages() {
  ...
  thumbnailImgContainer.forEach((container) => {
    ...
    imgs.forEach((img) => {
      if ([...img.classList].some((cls) => cls.startsWith("thumb-")))
        thumbImgs.push(img);
    });
  });
      ...
      if (forVal) {
        ...
        fileInput.addEventListener("change", (e) => {
          ...
          thumbImgs.forEach((thumbnailImg) => {
            if (
              (forVal === "upload-img-front" &&
                thumbnailImg.classList.contains("thumb-1")) ||
              (forVal === "upload-img-side" &&
                thumbnailImg.classList.contains("thumb-2")) ||
              (forVal === "upload-img-side-top" &&
                thumbnailImg.classList.contains("thumb-3")) ||
              (forVal === "upload-img-top-bottom" &&
                thumbnailImg.classList.contains("thumb-4"))
            ) {
...
function deleteThumbImages() {
  const labels = productModalContainer.querySelectorAll(
    ".thumbnail-img-wrapper .thumbnail-img-container label"
  );
...
const formModalTitle = document.querySelector(
  ".form-modal-container .form-modal-content .form-modal-header h3"
);
const productForm = document.querySelector(
  ".form-modal-container .form-modal-content .product-form"
);
const addProductDetailsBtn = document.querySelector(
  ".add-product-inner-container .details-container .details-header-container .add-product-btn"
);
const editProductBtn = document.querySelector(
  ".add-product-inner-container .product-details-container .details-header-container .edit-product-btn"
);

const additionalForm = document.querySelector(
  ".form-modal-container .form-modal-content .additional-form"
);
const addAditionalBtn = document.querySelector(
  ".add-product-inner-container .details-container .details-header-container .add-additional-btn"
);
const editAdditionalBtn = document.querySelector(
  ".add-product-inner-container .details-container .details-header-container .edit-additional-btn"
);

const formModalContent = formModalContainer.querySelector(
  ".form-modal-content"
);
...
formModalContainer.addEventListener("click", (e) => {
  if (!formModalContent.contains(e.target)) {
    ...
    delProductFormData();
    delAdditionalData();
    const inputWrappers = formModalContent.querySelectorAll(".input-wrapper");
...
const productDetailsHeaderContainer = productDetailsContainer.querySelector(
  ".details-header-container"
);
const productStatusImg = productDetailsHeaderContainer.querySelector("img");
const productStatusCount =
  productDetailsHeaderContainer.querySelector("p span");
const productDetailsBodyContainer = productDetailsContainer.querySelector(
...
function validateAdditionalForm() {
  const videoInput = additionalForm.querySelector("input[name='video_url']");
  const inputWrapper = videoInput.parentElement;
...
const addDetailsHeaderContainer = addDetailsContainer.querySelector(
  ".details-header-container"
);
...
const addDetailsBodyContainer = addDetailsContainer.querySelector(
  ".details-body-container"
);
...
function displayAddData(addData) {
  ...
  if (addDataLength === 0) {
    addTexts.forEach((addText) => {
      addText.querySelector("span").innerHTML = "";
    });
    addStatusImg.src = "./assets/images/completed tick-default.png";
    addStatusImg.alt = "completed tick-default";
  ...
  addTexts.forEach((addText) => {
    const fields = addText.querySelectorAll(
      ".add-brand, .add-model, [class^='view-']"
    );

    fields.forEach((el) => {
      let className = [...el.classList].find(
        (cls) => cls.startsWith("add-") || cls.startsWith("view-")
      );

      if (!className) return;

      let key = className.replace("view-", "");
...
const formModalCloseBtn = document.querySelector(
  ".form-modal-container .form-modal-content .form-modal-header .modal-close-btn"
);
...
const formSaveBtn = document.querySelector(
  ".form-modal-container .form-modal-content .modal-footer .save-btn"
);
const formCancelBtn = document.querySelector(
  ".form-modal-container .form-modal-content .modal-footer .cancel-btn"
);
...
formSaveBtn.addEventListener("click", (e) => {
  if (activeSection === "product") {
    ...
    if (validateProductForm()) {
      ...
      closeModal();
      editProductBtn.classList.add("error");
      addProductDetailsBtn.classList.add("not-active");
      updateSaveAndSubmitStatus();
    }
  } else if (activeSection === "additional") {
...
function updateSaveAndSubmitStatus() {
  ...
  if (checkUpdateConditions() && updateProductBtn) {
    ...
    submitProductsBtn.classList.remove("not-active");
  }
  if (checkUpdateConditions() && updateLocalProductBtn && isLocalProduct) {
...
function delAdditionalData() {
  ...
  const additionalInputs = document.querySelectorAll(
    ".additional-form input, .additional-form select"
  );
...
function scrollToTop() {
  ...
  modalMainWrapper.scrollTo({ top: 0, behavior: "smooth" });
}
...
const updateProductBtn = document.querySelector(
  ".add-product-outer-container .save-btn-container .update-btn"
);
updateProductBtn.classList.remove("active");
  ...
  updateProductBtn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://taskapi.devdews.com/api/products/update/${productData._id}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
const viewReviewBtn = document.querySelector(
  ".review-details-container .view-review-btn"
);
const reviewModalContainer = document.querySelector(".review-modal-container");
const reviewModalContent = reviewModalContainer.querySelector(
  ".review-modal-content"
);
const reviewModalCloseBtn =
  reviewModalContainer.querySelector(".modal-close-btn");
const reviewEditBtn = reviewModalContainer.querySelector(
  ".modal-footer .edit-btn"
);
const confirmBtn = reviewModalContainer.querySelector(
...
function processEditedProduct() {
  let recievedProducts = [];
  try {
    recievedProducts = JSON.parse(
      sessionStorage.getItem("savedProducts") || "[]"
    );
  } catch {
    recievedProducts = [];
  }
  const filteredDrafts = recievedProducts.filter(
    (p) => !(p.editing && p.isPosted)
  );
  savedProducts = filteredDrafts;

  const productToEdit = recievedProducts.find((p) => p.editing === true);
  if (productToEdit) {
...
const addProductContainerCloseBtn = document.querySelector(
  ".modal-main-container .close-btn-container .close-btn"
);
...
addProductContainerCloseBtn.addEventListener("click", () => {
  if (savedProducts.length > 0) {
    ...
    sessionStorage.setItem("savedProducts", JSON.stringify(cleanedProducts));
    // window.name = JSON.stringify(cleanedProducts);
  }
  isLocalProduct = false;
  const modalFormContainers = document.querySelectorAll(
    ".modal-main-container .form-container"
  );
  ...
  const prodTexts = productDetailsBodyContainer.querySelectorAll("p");
  prodTexts.forEach((prodText) => {
    prodText.querySelector("span").innerHTML = "";
  });
  productStatusImg.src = "./assets/images/completed tick-default.png";
  productStatusImg.alt = "completed tick-default";
  productStatusCount.innerHTML = "(0/9)";
  editProductBtn.classList.remove("active");
  addProductDetailsBtn.classList.remove("not-active");
  ...
  addStatusImg.src = "./assets/images/completed tick-default.png";
  addStatusImg.alt = "completed tick-default";
  addStatusCount.innerHTML = "(0/8)";
  editAdditionalBtn.classList.remove("active");
  addAditionalBtn.classList.remove("not-active");
  ...
  productImgEditBtn.classList.remove("active");
  uploadImgBtn.classList.remove("not-active");
  ProductImgStat.innerHTML = `(0/4)`;
  ProductImgStatImg.src = "./assets/images/completed tick-default.png";
  ProductImgStatImg.alt = "completed tick-default";
  thumbnailPreview.innerHTML = "";
  mainImageContainer.innerHTML = "";
  closeModal();
  removeProductModalDatas();
  addProductModalContainer.classList.remove("active");
  // setTimeout(() => unlockBody(), 200);
