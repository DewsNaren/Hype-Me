...
function getProducts() {
  let receivedObj;
  try {
    receivedObj = JSON.parse(
      sessionStorage.getItem("productSearchResults") || "{}"
    );
  } catch {
    receivedObj = {};
  }
...
function applyFilters(page = 1) {
  ...
  const minPercent =
    ((minVal - parseInt(minRange.min)) /
      (parseInt(minRange.max) - parseInt(minRange.min))) *
    100;
  const maxPercent =
    ((maxVal - parseInt(maxRange.min)) /
      (parseInt(maxRange.max) - parseInt(maxRange.min))) *
    100;
  ...
  function detectBrand(searchKeyword) {
    ...
    const allBrands = [
      ...new Set(totalProducts.map((p) => p.brand.toLowerCase())),
    ];
  ...
  const checkedModels = Array.from(
    document.querySelectorAll(".model-filter input:checked")
  ).map((input) => input.value);
  ...
  const checkedyears = Array.from(
    document.querySelectorAll(".year-filter input:checked")
  ).map((input) => input.value);
...
function renderProducts(products, page = 1, headerLabel = "All Shoes") {
  ...
  productList.innerHTML = `<div class="product-list-header"> 
                <h2>${headerLabel}<span>(Showing ${
    start + 1
  } - ${endIndex} products of ${filteredProducts.length} products)</span></h2> 
  ...
  paginatedProducts.forEach((product) => {
    productListBody.innerHTML += `<div class="product" data-product-id=${
      product._id
    }> 
...
function changeBrandFilters(totalProducts) {
  filterContainers.forEach((container) => {
    if (
      container.querySelector(".filter-title").textContent.trim() === "Brand"
    ) {
      ...
      if (totalBrands.length > 5) {
        ...
        brandMoreLink.onclick = (e) => {
          ...
          const parentContainer = brandList.closest(
...
function changeModelFilters(totalProducts) {
  filterContainers.forEach((container) => {
    if (
      container.querySelector(".filter-title").textContent.trim() === "Model"
    ) {
      ...
      if (totalModels.length > 5) {
        ...
        modelMoreLink.onclick = (e) => {
          ...
          const parentContainer = modelList.closest(
            ".filter-dropdown-container"
          );
...
function changeYearFilters(totalProducts) {
  filterContainers.forEach((container) => {
    if (
      container.querySelector(".filter-title").textContent.trim() ===
      "Release Year"
    ) {
      ...
      if (totalYears.length > 5) {
        ...
        yearMoreLink.onclick = (e) => {
          e.preventDefault();
          const parentContainer = yearList.closest(
            ".filter-dropdown-container"
          );
...
filterHeaders.forEach((header) => {
  header.addEventListener("click", (e) => {
    ...
    if (
      !container ||
      !container.classList.contains("filter-dropdown-container")
    )
      return;
...
document
  .querySelectorAll(".brand-list, .model-list, .year-list")
  .forEach((list) => {
