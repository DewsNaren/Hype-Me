const convofilterBtns = document.querySelectorAll(
  ".chat-conversations .conversation-header .convo-filter-container .convo-filter-btn"
);
const convoItems = document.querySelectorAll(
  ".chat-conversations .convo-list .convo-item"
);
...
const chatHeader = document.querySelector(
  ".chat-inner-container .chat-active .chat-header"
);
const activeImg = chatHeader.querySelector(".contact-info .product-img");
const activeName = chatHeader.querySelector(
  ".contact-info .contact-details .contact-user-info .contact-name"
);
const activeProductPreview = chatHeader.querySelector(
  ".contact-info .contact-details .contact-user-info .active-product-preview"
);
const activePrice = chatHeader.querySelector(
  ".contact-info .contact-details .contact-user-info .product-price"
);
...
let messages = JSON.parse(sessionStorage.getItem("storedMessages")) || [
  ...defaultMessages,
];
...
inputField.addEventListener("input", () => {
  if (inputField.value.trim() !== "") {
    ...
    if (!sendListenerAttached) {
      voiceBtn.addEventListener("click", handleSendMessage);
      sendListenerAttached = true;
    }
  } else {
    voiceBtn.src = "./assets/images/mic.png";
    voiceBtn.alt = "voice";
...
function storeMessage(messageVal) {
  ...
  sellerMsg = randomMessages[Math.floor(Math.random() * randomMessages.length)];
  sendMessage();
  messages[clicked].chat.push({
    from: "in",
    text: sellerMsg,
  ...
  sessionStorage.setItem(
    "storedMessages",
    JSON.stringify(messages) || [...defaultMessages]
  );
}
...
convoItems.forEach((convoItem, index) => {
  convoItem.addEventListener("click", () => {
    const selectedProductImg = convoItem.querySelector(
      ".convo-info .convo-detail .convo-product-picture"
    );
    activeImg.src = selectedProductImg.src;
    activeImg.alt = selectedProductImg.alt;

    const contactName = convoItem.querySelector(
      ".convo-info .convo-detail .convo-user-info .contact-name"
    );
    activeName.textContent = contactName.textContent;

    const productPreview = convoItem.querySelector(
      ".convo-info .convo-detail .convo-detail .convo-user-info .product-preview"
    );
  ...
  convoItems.forEach((item, index) => {
    ...
    if (chats.length > 0) {
      ...
      previewEl.innerHTML = `
        ${
          lastMsg.from === "out"
            ? `<img src="./assets/images/double-tick.svg" class="double-tick-icon">`
            : ""
        }
...
function renderMessage(clickedIndex) {
  ...
  requestAnimationFrame(() => {
    messagesContainers.scrollTop = messagesContainer.scrollHeight;
  });
}

function initializeChat() {
  if (sessionStorage.getItem("storedMessages")) {
    let clicked;
    convoItems.forEach((convoItem, index) => {
      if (convoItem.classList.contains("active")) {
        clicked = index;
    ...
    renderMessage(clicked);
    setTimeout(() => {
      const messagesContainers = document.querySelector(
        ".chat-active .chat-messages"
      );
      messagesContainers.scrollTop = messagesContainers.scrollHeight;
    }, 50);
  }
}
...
const chatCloseBtn = document.querySelector(
  ".chat-close-btn-container .chat-close-btn"
);
...
const chatConversations = document.querySelector(
  ".chat-inner-container .chat-conversations"
);
...
const chatSearch = chatConversations.querySelector(
  ".conversation-header .chat-search-wrapper input"
);

const contactNames = chatConversations.querySelectorAll(
  ".convo-item .convo-detail .contact-name"
);
let contactNameArr = [];
